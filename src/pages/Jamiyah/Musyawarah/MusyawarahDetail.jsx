...
const DetailMusyawarah = () => {
  ...
  useEffect(() => {
    const fetchMusyawarahData = async () => {
      ...
      try {
        ...
        setTotal(totalData);
        setIsPimpinanCabang(
          fetchedMusyawarah[0]?.tingkat_musyawarah === "pimpinan_cabang"
        );
      } catch (error) {
  ...
  const handleDelete = async (id_musyawarah, id_detail) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus data ini?")) {
      try {
        await api.delete(
          `${API_URL}/musyawarah/detail/${id_musyawarah}/${id_detail}`
        );
          ...
          <div className="bg-green-800 p-4 md:p-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <h1 className="text-xl md:text-2xl font-bold text-white">
                Detail{" "}
                {isPimpinanCabang ? "Tasykil Pimpinan Cabang" : "Musyawarah"}
              </h1>
              <button
                onClick={handleBack}
          ...
          <div className="p-4 md:p-6">
            {musyawarah.length > 0 ? (
              musyawarah.map((item, index) => (
                <div
                  key={item.id_musyawarah || index}
                  className="mb-6 last:mb-0"
                >
                  <div className="bg-green-700 p-3 md:p-4 rounded-t-lg text-white">
                    <h2 className="text-lg md:text-xl font-bold">
                      {isPimpinanCabang
                        ? `Tasykil #${item.id_musyawarah}`
                        : `Musyawarah #${item.id_musyawarah}`}
                    </h2>
                    {!isPimpinanCabang && (
                      <p className="text-xs md:text-sm">
                        {item.master_jamaah?.nama_jamaah ||
                          "Tidak ada data nama jamaah"}
                      </p>
                    )}
                  </div>
                      ...
                      {[
                        ...(!isPimpinanCabang
                          ? [
                              {
                                label: "Nama Jamaah",
                                value: item.master_jamaah?.nama_jamaah,
                              },
                            ]
                          : []),
                        {
                          label: "Nomor Musyawarah",
                          value: item.id_musyawarah,
                        },
                        { label: "No SK", value: item.no_sk },
                        {
                          label: "Tanggal Pelaksanaan",
                          value: formatTanggal(item.tgl_pelaksanaan),
                        },
                        {
                          label: "Tanggal Akhir Jihad",
                          value: formatTanggal(item.tgl_akhir_jihad),
                        },
                        {
                          label: "Status",
                          value: item.aktif ? (
              ...
              <div className="text-center py-8 md:py-12">
                ...
                <p className="text-gray-500 mt-2 text-sm md:text-base">
                  Tidak ditemukan data musyawarah yang sesuai.
                </p>
                <Link
          ...
          <div className="p-6 bg-white rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-lg font-bold">
                Daftar{" "}
                {isPimpinanCabang ? "Anggota Tasykil" : "Anggota Musyawarah"}
              </h1>
              {hasPermission("add_anggota_musyawarah") && (
                <Link
                        ...
                        >
                          ...
                          <td className="border p-2 text-center">
                            <div className="flex justify-center gap-2">
                              {/* Edit Button */}
                              {hasPermission("edit_anggota_musyawarah") && (
                                <Link
                                  to={`/jamiyah/musyawarah/detail/edit/${anggota.id_musyawarah}/${item.id_musyawarah_detail}`}
                                >
                              ...
                              {/* Delete Button */}
                              {hasPermission("delete_anggota_musyawarah") && (
                                <button
                                  onClick={() =>
                                    handleDelete(
                                      anggota.id_musyawarah,
                                      item.id_musyawarah_detail
                                    )
                                  }
